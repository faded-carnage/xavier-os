...
import { 
  ...
  ShieldAlert, Cpu, Rocket, ShieldX
} from 'lucide-react';
...
export default defineConfig({
  ...
  build: { outDir: 'dist' }
        ...
        "index.html": """<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><title>Xavier OS</title><script src="https://cdn.tailwindcss.com"></script><style>body{background:#010101;color:#fff;overflow:hidden;margin:0;}</style></head><body><div id="root"></div><script type="module" src="/index.tsx"></script></body></html>""",
...
-- 2. Enable Row Level Security (Optional: Open for demo)
alter table social_feed enable row level security;
create policy "Public Access" on social_feed for all using (true);
""",
        "types.ts": """export type ModuleType = 'dashboard' | 'storage' | 'vpn' | 'aistudio' | 'social' | 'store' | 'marketing' | 'agent' | 'adcreatives' | 'collab' | 'cyber' | 'intel' | 'uplink' | 'matrix' | 'mission' | 'forge' | 'settings' | 'launch' | 'publish' | 'admin' | 'stripe-nexus' | 'server-mgmt' | 'swarm' | 'revenue' | 'talent' | 'viral-blast';
export interface UserSession { email: string; role: 'OWNER' | 'ADMIN' | 'OPERATIVE'; isUnlimited: boolean; signature: string; }""",
        "metadata.json": """{ "name": "Xavier AI Ecosystem", "description": "Military-grade cloud ecosystem.", "requestFramePermissions": ["camera", "microphone", "geolocation"] }""",
        "README.md": "# Xavier Sovereign OS Rebuild V4.2.7\\n\\n## 1. Code Repository (GitHub)\\nPush this entire folder to your private GitHub repository.\\n\\n## 2. Database (Supabase)\\n- Create a new project at Supabase.com\\n- Go to SQL Editor and run the content of `supabase_schema.sql`\\n- Get your URL and Anon Key from Project Settings\\n\\n## 3. Deployment (Vercel)\\n- Import from GitHub\\n- Add Environment Variables:\\n   - API_KEY (Gemini)\\n   - VITE_SUPABASE_URL\\n   - VITE_SUPABASE_ANON_KEY"
    }
    ...
    for path, content in files.items():
        ...
        with open(path, "w", encoding="utf-8") as f:
            f.write(content)
        print(f"[FILE] Rebuilt: {path}")

    print("\\n" + "="*65)
    print("   SUCCESS: WORLD-CLASS REBUILD COMPLETED (V4.2.7)")
    print("="*65)
    print("   NEXT STEPS:")
    print("   1. cd " + PROJECT_DIR)
    print("   2. git init")
    print("   3. git remote add origin https://github.com/YOUR_GITHUB_USER/xavier-os.git")
    print("   4. git push -u origin main")
    print("   5. Setup Supabase Tables using 'supabase_schema.sql'")
    print("="*65 + "\\n")
                   ...
                   <div className="p-14 bg-white/5 border border-white/10 rounded-[4rem] space-y-12 shadow-inner group hover:border-cyan-500/30 transition-all relative overflow-hidden">
                      ...
                      <div className="grid grid-cols-2 gap-12 relative z-10">
                         <div className="p-8 bg-black/40 rounded-[2.5rem] border border-white/5 space-y-4">
                            <p className="text-[10px] font-bold text-white/20 uppercase tracking-widest">TLS_PROTOCOL</p>
                            <p className="text-2xl font-bold text-cyan-400 font-mono">AES-XTS-512_ACTIVE</p>
                         </div>
                         <div className="p-8 bg-black/40 rounded-[2.5rem] border border-white/5 space-y-4">
                            <p className="text-[10px] font-bold text-white/20 uppercase tracking-widest">SERVER_UPTIME</p>
